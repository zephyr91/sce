...
-- 4) 5 piores/melhores produtos/serviços avaliados

select max(media),nomeProduto from AVALIACAO a, AVALIACAO_PRODUTO ap, PRODUTO p where a.idAvaliacao=ap.idAvaliacao and p.idProduto=ap.idProduto group by nomeProduto order by media desc limit 0, 5;

select min(media),nomeProduto from AVALIACAO a, AVALIACAO_PRODUTO ap, PRODUTO p where a.idAvaliacao=ap.idAvaliacao and p.idProduto=ap.idProduto group by nomeProduto order by media limit 0, 5;

select max(media),nomeServico from AVALIACAO a, AVALIACAO_SERVICO aser, SERVICO s where a.idAvaliacao=aser.idAvaliacao and s.idServico=aser.idServico group by nomeServico order by media desc limit 0, 5;

select min(media),nomeServico from AVALIACAO a, AVALIACAO_SERVICO aser, SERVICO s where a.idAvaliacao=aser.idAvaliacao and s.idServico=aser.idServico group by nomeServico order by media limit 0, 5;

-- 5) Pergunta que tem a maior/menor média

	-- Média das notas de cada questão onde o produto é = '$idprodutoescolhido'
SELECT SUM(r.nota)/COUNT(r.nota), eq.texto FROM RESPOSTA r, ESTRUTURA_QUESTAO eq, AVALIACAO_PRODUTO ap, AVALIACAO a WHERE r.idEstruturaQuestao = eq.idEstruturaQuestao AND a.idAvaliacao = ap.idAvaliacao AND a.idAvaliacao = r.idAvaliacao AND ap.idProduto = '$idprodutoescolhido' GROUP BY r.idEstruturaQuestao;

	-- Média das notas de cada questão onde o serviço é = '$idservicoescolhido'

SELECT SUM(r.nota)/COUNT(r.nota), eq.texto FROM RESPOSTA r, ESTRUTURA_QUESTAO eq, AVALIACAO_SERVICO aser, AVALIACAO a WHERE r.idEstruturaQuestao = eq.idEstruturaQuestao AND a.idAvaliacao = aser.idAvaliacao AND a.idAvaliacao = r.idAvaliacao AND aser.idServico = '$idservicoescolhido' GROUP BY r.idEstruturaQuestao;
